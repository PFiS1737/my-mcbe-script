import { world, GameMode, DisplaySlotId, ObjectiveSortOrder } from "@minecraft/server"
...
export class Handler {
    ...
    async add({ objectiveId, criteria, displayName = objectiveId }) {
        if (world.scoreboard.getObjective(objectiveId)) return { code: 0, message: "DUPLICATE_OBJECTIVE" }
        
        const [ criteriaType ] = parseCriteria(criteria)
        if (!CRITERIA.get(criteriaType)) return { code: 0, message: "UNKNOWN_CRITERIA" }
        ...
        await asyncRun(() => {
            ...
            if (!objective.hasParticipant(this.player)) {  // TODO: 或许放到 start 里更合理
    ...
    async start({
        objectiveId,
        criteria = globalDB.get(objectiveId)
    }) {
