...
/* module.exports = */ export function serialize(obj, options) {
  options || (options = {})
  ...
  function replacer(key, value) {
    ...
    if (type === "object") {
      ...
      if (Array.isArray(origValue)) {
        const isSparse =
          origValue.filter(() => true).length !== origValue.length
        if (isSparse) {
    ...
    if (
      type === "number" &&
      !Number.isNaN(origValue) &&
      !Number.isFinite(origValue)
    ) {
  ...
  if (
    ...
    arrays.length === 0 &&
    undefs.length === 0 &&
    infinities.length === 0 /* && bigInts.length === 0 && urls.length === 0 */
  ) {
    return str
  }
  ...
  return str.replace(
    PLACE_HOLDER_REGEXP,
    (match, backSlash, type, valueIndex) => {
