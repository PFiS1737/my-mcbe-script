 ...
 */
import { system, world, Dimension, ItemComponentTypes, BlockPermutation, BlockVolume, ItemStack, MinecraftDimensionTypes as MinecraftDimensionTypes$1, EntityDamageCause, DisplaySlotId, ObjectiveSortOrder, GameMode, Player } from '@minecraft/server';
...
class ItemStackWithDurability extends WrappedItemStack {
    ...
    static assert(itemStack) {
        if (ItemStackWithDurability.match(itemStack)) return true;
        throw new TypeError(`The "${itemStack.typeId}" doesn't have the "${ItemComponentTypes.Durability}" component.`);
    }
    get damage() {
        return this.components.get(ItemComponentTypes.Durability).damage;
    }
    set damage(value) {
        this.components.get(ItemComponentTypes.Durability).damage = value < 0 ? 0 : value;
    }
    get maxDurability() {
        return this.components.get(ItemComponentTypes.Durability).maxDurability;
    }
    get durability() {
    ...
    applyDamage(damage) {
        const unbreakingLevel = this.enchants.getEnchantment(MinecraftEnchantmentTypes.Unbreaking)?.level;
        const probability = this.components.get(ItemComponentTypes.Durability).getDamageChance(unbreakingLevel) / 100;
...
option$3.applyMainPlayer().then(()=>{
    for (const player of world.getAllPlayers()){
        option$3.applyPlayer(player);
    }
}).then(()=>option$3.init()).then(()=>setupListener$1()).catch(BetterConsole.error);
...
let Handler$1 = class Handler {
    ...
    constructor(player){
        ...
        const playerDB = ALL_PLAYER_DATABASES$1.get(player);
...
option$2.applyMainPlayer().then(()=>{
    for (const player of world.getAllPlayers()){
        option$2.applyPlayer(player);
    }
}).then(()=>option$2.init()).then(async ()=>{
    for (const [player] of option$2.players){
...
option$1.applyMainPlayer().then(()=>{
    for (const player of world.getAllPlayers()){
        option$1.applyPlayer(player);
    }
}).then(()=>option$1.init()).then((optMap)=>{
    // 将所有玩家的数据库实例化并储存在 ALL_PLAYER_DATABASES 中
...
const setupListener = ()=>world.afterEvents.playerBreakBlock.subscribe((event)=>{
        ...
        player.useMainHandItem(async (mainHandItem)=>{
            ...
            const wrappedItem = ItemStackWithDurability.match(mainHandItem) ? new ItemStackWithDurability(mainHandItem) : new WrappedItemStack(mainHandItem);
            let totalDamage = 0;
            const totalItems = [];
            let totalXp = 0;
            while(blockList.size > 0 && (!playerOption.getItemVal("prevent_tool_destruction") || playerOption.getItemVal("prevent_tool_destruction") && (wrappedItem instanceof ItemStackWithDurability ? totalDamage < wrappedItem.durability : true))){
